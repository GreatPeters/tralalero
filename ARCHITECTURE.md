...
- `NoryangjinMapToolWindow` separates placement into `오브젝트`, `적군`, and `기믹`. Its enemy palette exposes the five Forward enemy prefabs, places them under `Noryangjin_MapTool/Enemies`, and gives enemies an occupancy layer independent from scenery.
...
- Runtime compatibility stays with `EnemyScript_space`: trigger `act` enters `Attack`, the attack returns to `Idle` after exit time, and trigger `die` enters terminal state `Die`.
