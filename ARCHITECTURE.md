...
- `PlayerStatusHudBuilder` adds the rebuildable screen-space `PlayerStatusHUD` contract to that Canvas. The slim parchment cards show live current/max health and attack, stay hidden before gameplay, and replace the legacy player-child health Canvas plus `ATT` text only when all HUD references are valid.
