...
- `NoryangjinMapToolWindow` separates placement into `오브젝트`, `적군`, `기믹`, and `보너스`. Its enemy palette exposes the five Forward enemy prefabs under `Noryangjin_MapTool/Enemies`; the bonus palette exposes the playable wall prefabs under `Noryangjin_MapTool/Bonuses` and adds the Noryangjin marker that suppresses legacy global-volume lookup. The marker distinguishes persistent map-authored walls from transient enemy-drop walls, so stage preparation clears only the latter. Enemy and bonus placements each use occupancy layers independent from scenery.
