...
- `Assets/ShooterSurvival/Scenes/Tools/Noryangjin_MapTool_Mode_SR18.unity` is a separate build-excluded roads-only sibling for the SR18 Lighthouse Infinity plan. It preserves Map 1's 51-road prefix, adds 179 roads and 10 turn spots, and leaves decoration/encounters empty. Its two elevated authoring spans are not runtime-traversable until player Y-following is implemented.
...
- Triggered projectile delay advances only while the custom game-running flag and time factor advance. Movement activation rejects zero speed away from the target, a destroyed target cancels to `idle` with one warning, legacy serialized mode value `3` normalizes to move-then-attack, and visual facing composes the prefab Animator's authored rotation offset instead of overwriting it.
