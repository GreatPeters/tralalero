    ...
    [Test] public void AuthoredChapter_MatchesWorkbookAndHasAllSixCombatModels()
    {
        ...
        try
        {
            ...
            Vector3 SampleAuthored(float distance,out Vector3 forward)
            {
                ...
                continuous.Sample(Mathf.Max(0,distance),false,out var point,out forward);
                return distance<0?point+forward*distance:point;
            }
            float previousHeight=.12f;
            for(float d=0;d<=(continuous!=null?continuous.length:HighwaySceneBuilder.Length);d+=4)
            {
                var route=SampleAuthored(d,out var forward);var proposed=route;proposed.y=previousHeight;
                Assert.That(follower.TryProjectPosition(proposed,forward,out var supported),Is.True,"Height follow at "+d);
                // Verify against the authored curve, retaining tolerance at mesh joins.
