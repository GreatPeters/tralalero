...
public sealed class Sr18RuntimeFixSceneTests
{
    [Test] public void ExitAndLegacyScenery_AreSafeWithoutChangingRoadGeometry()
    {
        ...
        try
        {
            ...
            Assert.That(map.Find("Roads").childCount,Is.EqualTo(230));Assert.That(map.Find("Props").childCount,Is.EqualTo(717));
            ...
            {
                Assert.That(lamp.gameObject.activeInHierarchy,Is.True);Assert.That(lamp.enabled,Is.False);
            ...
            Assert.That(canvas.youWinUI.transform.Find("Next Level").gameObject.activeSelf,Is.True);
        }
