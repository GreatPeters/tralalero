...
- In the Noryangjin map tool's selection mode, each placed object's `Y` height label is a clickable selection target. The `설치 조정` preview follows the selected placement root first and falls back to the object under the map cursor when nothing is selected. The blue footprint belongs only to the currently selected placement root (including a selected child); clearing selection also clears the blue footprint. The last-placed fallback remains limited to `이어 복붙`.
