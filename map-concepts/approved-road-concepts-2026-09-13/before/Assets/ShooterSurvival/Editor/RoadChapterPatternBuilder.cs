...
public static class RoadChapterPatternBuilder
{
    ...
    { string path = Art + "/" + name + ".mat"; var m = AssetDatabase.LoadAssetAtPath<Material>(path); if (m != null) return m; m = new Material(Shader.Find("Universal Render Pipeline/Lit")) { color = color, enableInstancing = true }; m.SetFloat("_Smoothness", .2f); AssetDatabase.CreateAsset(m, path); return m; }
