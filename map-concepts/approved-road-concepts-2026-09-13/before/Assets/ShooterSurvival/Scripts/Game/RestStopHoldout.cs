...
public sealed class RestStopHoldout : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        hud?.Show(this, $"식당 포위전  {Mathf.CeilToInt(Duration - Elapsed):00}초", phase == 0 ? "출입구 경계  가까운 적 자동 조준" : phase == 1 ? "양쪽에서 접근 중  뒷문 개방까지 버티세요" : "사방에서 접근 중  곧 탈출로가 열립니다", 3);
    ...
    private void LateUpdate()
    {
        ...
        if (cameraCaptured)
        {
            // A stable route frame shows the rear and side entrances while the body aims.
            battleCamera.transform.localPosition = Vector3.Lerp(battleCamera.transform.localPosition, new Vector3(0, 24, -18), Time.unscaledDeltaTime * 3);
            battleCamera.transform.LookAt(center.position + center.forward * 2);
            battleCamera.fieldOfView = Mathf.Lerp(battleCamera.fieldOfView, 64, Time.unscaledDeltaTime * 3);
        }
