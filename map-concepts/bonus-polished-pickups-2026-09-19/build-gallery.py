...
topic="bonus-polished-pickups-2026-09-19"
...
gallery=root/"tmp/image-previews/bonus-gallery-2026-09-19/polished"
...
template=template.replace("보너스 시안 모아보기","입체 보너스 · 세 가지 방향")
template=template.replace("이미지를 눌러 크게 보세요. 원본 크기 보기와 PNG 저장도 가능합니다.","01 능력 휘장 · 02 신발 개조 모듈 · 03 성장 카드 — 게임 안 모습, 형태, 획득 연출을 비교하세요.")
nav='<nav aria-label="시안 선택"><button data-group="all" aria-pressed="true">전체 3안</button>'+''.join(f'<button data-group="{c["id"]}" aria-pressed="false">{html.escape(c["id"]+" "+c["name"])}</button>' for c in data["concepts"])+'</nav>'
...
print("Saved 3 selected images; gallery:",gallery/"index.html")
