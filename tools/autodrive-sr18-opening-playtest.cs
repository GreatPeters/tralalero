...
string folder = UnityEditor.SessionState.GetString("SR18.LivePlaytest.20260909.folder", "") + "/opening";
...
steer = () =>
{
    ...
    if (Time.time - started >= 110)
    {
        ScreenCapture.CaptureScreenshot(folder + "/110-seconds.png");
    ...
    {
        nextCapture = UnityEditor.EditorApplication.timeSinceStartup + 2;
...
return new { started = true, folder, health = p.currentHealth, attack = p.currentDamage, timeScale = Time.timeScale, stopAfterGameSeconds = 110 };
