...
public static class CapturePolishedTalisman
{
    ...
    static IEnumerator Capture(string mode,PlayerScript player,Camera camera)
    {
        ...
        var folder="tmp/image-previews/talisman-polish-2026-09-20/"+(mode=="final"?"final/":"")+SceneManager.GetActiveScene().name;
        ...
        effect.Sample(.24f);yield return Shot(folder+"/02-unfold.png");
        effect.Sample(.50f);yield return Shot(folder+"/03-absorb.png");
        effect.Sample(.81f);yield return Shot(folder+"/04-body-pulse.png");
        effect.Sample(BonusTalismanPickup.Duration+.05f);UnityEngine.Object.Destroy(effect.gameObject);
