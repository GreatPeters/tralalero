...
public static class ChapterThemeBaseline20260917{public static object Main(){
 if(EditorApplication.isPlayingOrWillChangePlaymode)throw new Exception("Edit Mode required");
 string folder="tmp/backups/chapter-ui-themes-2026-09-17";if(Directory.Exists(folder))throw new Exception("Preserve existing baseline");Directory.CreateDirectory(folder);
 ...
 Directory.CreateDirectory("map-concepts/chapter-ui-themes-2026-09-17");File.WriteAllLines("map-concepts/chapter-ui-themes-2026-09-17/baseline.txt",lines);return string.Join("\n",lines);
}}
