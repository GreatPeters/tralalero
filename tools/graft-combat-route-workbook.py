...
backup=ROOT/'map-concepts/combat-route-fixes-2026-09-20/before/Data.xlsx'
corner='--corner-fix' in sys.argv
if corner:backup=ROOT/'tmp/combat-route-workbook/before-corner-fix.xlsx'
...
original=package(source);candidate=package(ROOT/'tmp/combat-route-workbook'/('corner-candidate.xlsx' if corner else 'candidate.xlsx'))
def sheets(parts):
...
changes=json.loads((ROOT/'tmp/combat-route-workbook'/('corner-changes.json' if corner else 'changes.json')).read_text(encoding='utf8'))
...
(ROOT/'map-concepts/combat-route-fixes-2026-09-20'/('workbook-corner-preservation.json' if corner else 'workbook-preservation.json')).write_text(json.dumps(report,indent=2),encoding='utf8')
