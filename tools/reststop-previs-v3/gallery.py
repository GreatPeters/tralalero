...
ROOT=Path.cwd();OUT=ROOT/'outputs/reststop-blender-v3-2026-09-23';G=ROOT/'tmp/image-previews/reststop-blender-v3-2026-09-23';G.mkdir(exist_ok=True)
...
sheet.save(G/'01-10-contact-sheet.png');copy(OUT/'vehicle-lineup.png')
downloads=[]
for file,label in [('reststop-v3.blend','수정된 Blender 원본'),('reststop-v3-environment.glb','정적 환경 GLB')]:
...
page=f'<!doctype html><html lang="ko"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>휴게소 수정 이미지 01–10</title><style>{css}</style><main><header><div class="eyebrow">REST STOP / BLENDER / REVISION 03</div><h1>정해진 경로를 전진하고,<br>그 안에서 좌우로 이동</h1><p>이동 카트를 제거하고 차량 6종을 섞었습니다. 카메라는 정해진 경로를 따라가고 상어만 좌우로 움직입니다. 아래는 수정된 Blender 원본에서 촬영한 1–10번 이미지입니다.</p><div class="legend"><span class="gold">노란 점선·화살표: 자동 진행 경로</span><span class="blue">파란 경계: 좌우 이동 통로</span></div></header>'
...
page+='<section><h2>차량 6종</h2><p>승용차 · 빨간 소형차 · 초록 SUV · 배송 트럭 · 노란 박스 트럭 · 파란 버스. 총 91대를 배치했습니다.</p><a href="vehicle-lineup.png" target="_blank"><img src="vehicle-lineup.png" alt="실제로 배치한 차량 여섯 종류"></a></section><div class="downloads">'+''.join(downloads)+'</div><footer>1600×900 PNG 10장 · 바닥 안내선은 이동 규칙을 확인하기 위한 프리비즈 표시입니다.<br>5번은 제자리 방어 구간입니다. Blender 원본에는 수정된 300초 타임라인이 들어 있습니다. 이번 요청에 맞춰 정지 이미지를 제작했습니다. Unity 씬 설치와 TRELLIS·Seedance 실행은 하지 않았습니다.</footer></main></html>'
(G/'index.html').write_text(page,encoding='utf8');(G/'gallery-status.json').write_text(json.dumps({'images':10,'source':'native Blender v3','video_rendered_this_revision':False},indent=2));print('GALLERY_READY',str(G),flush=True)
