...
OUT=Path.cwd()/'outputs/reststop-blender-v3-2026-09-23';bpy.ops.wm.open_mainfile(filepath=str(OUT/'reststop-v3.blend'));s=bpy.context.scene;p=bpy.data.objects['PLAYER_ROOT'];rail=bpy.data.objects['AUTO_RAIL_ANCHOR'];cam=s.camera
...
(OUT/'validation.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf8');print('V3_VALIDATED',json.dumps(report),flush=True)
...
for o in s.objects:
 ...
 o.select_set(True)
bpy.ops.export_scene.gltf(filepath=str(OUT/'reststop-v3-environment.glb'),export_format='GLB',use_selection=True,export_animations=False,export_apply=True,export_cameras=False,export_lights=False);print('V3_EXPORTED',flush=True)
