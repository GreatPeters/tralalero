...
def rail_frame(t):
 i=stage_index(max(0.,min(300.,t)));p=center_route(t)
 if i==4:return p,(0.,1.),(-1.,0.)
 elapsed=max(0.,t-STAGES[i][2]);travel=LENGTHS[i]/SPEED;pause_at=travel*(.12 if i==3 else .48);hold=max(0.,HOLDS[i])
 moving=elapsed if elapsed<pause_at else pause_at if elapsed<pause_at+hold else elapsed-hold
 d=max(0.,min(LENGTHS[i],moving*SPEED));a=sample_distance(i,d-.5);b=sample_distance(i,d+.5);dx=b[0]-a[0];dy=b[1]-a[1];n=max(1e-9,hypot(dx,dy));forward=(dx/n,dy/n)
 return p,forward,(-forward[1],forward[0])

def lane_offset(t):
 if 100<=t<=160:return 0.
 offset=0.
 for when,direction in [(9,1),(31.5,-1),(35.5,-1),(43.5,-1),(47,-1),(57,1),(87,-1),(179,1),(207,-1),(235,1),(263,1),(288,-1)]:
  u=(t-(when-1.1))/2.2
  if 0<u<1:offset+=direction*2.2*sin(pi*u)**2
 return max(-2.2,min(2.2,offset))

def route(t):
 p,f,n=rail_frame(t);offset=lane_offset(t);return p[0]+n[0]*offset,p[1]+n[1]*offset
