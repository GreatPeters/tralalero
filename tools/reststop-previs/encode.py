...
ROOT=Path.cwd();OUT=ROOT/'outputs/reststop-blender-300s-2026-09-23';VIDEOS=OUT/'videos';VIDEOS.mkdir(exist_ok=True)
...
NOTES=[
    '고속도로 분기 → 안내 표지 → 진입 차단기',
    '주차 차량 사이 이동 · 후진 예고와 회피',
    '간식 매대 · 벤치 · 도망가는 방문객 · 이동 카트',
    '자동문 개방 → 넓은 중앙 광장 진입',
    '위치 고정 · 초당 최대 90도 연속 회전 · 사방 방어',
    '식사 공간과 주문대 · 움직이는 배식 카트',
    '진열대 · 냉장고 · 계산대 · 카트 옆으로 우회',
    '600㎡ 화장실 · 청소 카트 · 물기 구역',
    '주유기 사이 통과 · 교차 차량을 피해 출구로',
    '추격 적 정리 → 차단기 개방 → 출구 통과',
]
...
for i,(idx,label,a,b) in enumerate(STAGES):
    ...
    while not ((OUT/'frames'/f'frame-{last:05d}.jpg').exists() and ((OUT/'frames'/f'frame-{last+1:05d}.jpg').exists() or (last==7200 and (OUT/'render-completed.json').exists()))):
    ...
    run(['-v','warning','-framerate',str(FPS),'-start_number',str(first),'-i',str(OUT/'frames/frame-%05d.jpg'),'-frames:v',str(last-first+1),'-vf',','.join(vf),'-an','-c:v','libx264','-threads','2','-preset','fast','-crf','20','-pix_fmt','yuv420p','-movflags','+faststart','-y',str(path)])
