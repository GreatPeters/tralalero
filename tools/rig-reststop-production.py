"""Rejected rest-stop geodesic experiment; use rig-reststop-proxy.py for new candidates."""
...
args=parser.parse_args(sys.argv[sys.argv.index('--')+1:]);out=Path(args.output).resolve()
compact = args.name in ('ParkingMarshal','SnackChef','CoffeeVendor')
actual_z = np.array([0,.08,.17,.30,.37,.45,.49,.53,.58,1.0])
canonical_z = np.array([0,.065,.20,.35,.43,.51,.595,.61,.675,1.0])
def physical(point):
    x,y,z = point
    return (x,y,float(np.interp(z,canonical_z,actual_z)) if compact else z)
...
if compact:coords[:,2]=np.interp(coords[:,2],actual_z,canonical_z)
# Locate the separate gloves from the actual generated albedo. This avoids
# placing a wrist in the palm when a mascot has unusually large hands.
gloves={}
if body.data.materials and body.data.uv_layers:
    shader=body.data.materials[0].node_tree.nodes.get('Principled BSDF')
    links=shader.inputs['Base Color'].links if shader else []
    texture=links[0].from_node if links and links[0].from_node.type=='TEX_IMAGE' else None
    if texture and texture.image:
        image=texture.image;pixels=np.array(image.pixels[:],dtype=np.float32).reshape(image.size[1],image.size[0],4)
        rgb=np.zeros((len(coords),3));hits=np.zeros(len(coords))
        for loop in body.data.loops:
            u,v=body.data.uv_layers.active.data[loop.index].uv
            color=pixels[min(image.size[1]-1,max(0,int(v*image.size[1]))),min(image.size[0]-1,max(0,int(u*image.size[0]))),:3]
            rgb[loop.vertex_index]+=color;hits[loop.vertex_index]+=1
        rgb/=np.maximum(hits[:,None],1)
        for side,sign in [('L',1),('R',-1)]:
            neutral=(rgb.max(axis=1)-rgb.min(axis=1))<.20
            tone=rgb.mean(axis=1)<.16 if args.name=='Police' else rgb.mean(axis=1)>.62
            mask=(coords[:,0]*sign>.22)&(coords[:,2]>.23)&(coords[:,2]<.52)&neutral&tone
            if mask.sum()>20:
                samples=coords[mask]
                lo=samples.min(axis=0)-np.array((.025,.025,.012))
                hi=samples.max(axis=0)+np.array((.025,.025,.015))
                region=np.all((coords>=lo)&(coords<=hi),axis=1)&(coords[:,0]*sign>.205)
                gloves[side]=np.where(region)[0]
...
for side,sign in [('L',1),('R',-1)]:
    ...
    hand_tail=(wrist[0]+sign*.02,wrist[1]-.015,wrist[2]-.065)
    if side in gloves:
        sample=coords[gloves[side]]
        wrist=tuple(np.median(sample[sample[:,2]>=np.quantile(sample[:,2],.75)],axis=0))
        hand_tail=tuple(np.median(sample[sample[:,2]<=np.quantile(sample[:,2],.30)],axis=0))
        elbow=tuple((np.array(shoulder)+np.array(wrist))*.5+np.array((sign*.015,0,0)))
    ...
    specs.update({'UpperArm.'+side:(shoulder,elbow,'Chest'),'Forearm.'+side:(elbow,wrist,'UpperArm.'+side),
        'Hand.'+side:(wrist,hand_tail,'Forearm.'+side),
...
for name,(a,b,parent) in specs.items():
    bone=armature.edit_bones.new(name);bone.head=Vector(physical(a))*height;bone.tail=Vector(physical(b))*height
...
torso_half={'SnackChef':.18,'Police':.155,'ParkingMarshal':.155}.get(args.name,.145)
def sstep(a,b,value):
...
for side,indices in gloves.items():
    wrist=np.array(specs['Hand.'+side][0]);tip=np.array(specs['Hand.'+side][1]);delta=tip-wrist
    for index in indices:
        vertex=body.data.vertices[int(index)]
        t=float(np.dot(coords[int(index)]-wrist,delta)/max(np.dot(delta,delta),1e-9))
        hand=sstep(-.15,.30,t)
        for group in list(vertex.groups):body.vertex_groups[group.group].remove([int(index)])
        if hand>0:groups['Hand.'+side].add([int(index)],hand,'REPLACE')
        if hand<1:groups['Forearm.'+side].add([int(index)],1-hand,'REPLACE')
# UV seam duplicates must deform together after the glove correction too.
shared=np.zeros((len(points),len(names)));shared_count=np.zeros(len(points))
for vertex in body.data.vertices:
    key=int(inverse[vertex.index]);shared_count[key]+=1
    for g in vertex.groups:shared[key,names.index(body.vertex_groups[g.group].name)]+=g.weight
shared/=np.maximum(shared_count[:,None],1)
for vertex in body.data.vertices:
    row=shared[int(inverse[vertex.index])];best=np.argsort(row)[-4:];total=row[best].sum()
    for g in list(vertex.groups):body.vertex_groups[g.group].remove([vertex.index])
    for index in best:
        if row[index]>.0001:groups[names[int(index)]].add([vertex.index],float(row[index]/total),'REPLACE')
weight_errors=[abs(sum(g.weight for g in v.groups)-1) for v in body.data.vertices]
assert max(weight_errors)<.002 and all(1<=len(v.groups)<=4 for v in body.data.vertices), 'Invalid skin weights'
...
report={'name':args.name,'source':args.source,'height':height,'triangles':sum(len(p.vertices)-2 for p in body.data.polygons),'bones':len(armature.bones),'actions':durations,'skeleton':{n:(physical(a),physical(b),p) for n,(a,b,p) in specs.items()},'maximumGroundError':max(abs(c['ground']) for c in checks),'rigged':True,'equipmentPendingNativeBinding':True,'compact_proportions':compact,'materials':'original TRELLIS PBR maps retained'}
